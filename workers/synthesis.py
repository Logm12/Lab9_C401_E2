...
SYSTEM_PROMPT = """Bạn là trợ lý IT Helpdesk nội bộ sử dụng model GPT-5.4-mini.

QUY TẮC NGHIÊM NGẶT:
1. CHỈ trả lời dựa vào context được cung cấp. KHÔNG dùng kiến thức ngoài.
2. Nếu context không đủ để trả lời → Bắt buộc nói rõ: "Không đủ thông tin trong tài liệu nội bộ".
3. Trích dẫn nguồn (Citation): Trích dẫn tên file theo định dạng [tên_file.txt] ngay sau thông tin lấy từ file đó.
4. Trả lời súc tích, có cấu trúc. Nếu có exceptions từ policy_result, phải nêu rõ ràng trước khi kết luận.
"""


def _call_llm(messages: list) -> str:
    """
    Gọi LLM để tổng hợp câu trả lời (TIP-010: Pure OpenAI only).
    """
    try:
        ...
        return f"[SYNTHESIS ERROR] Model: gpt-4o-mini. Lỗi: {str(e)}"
...
def run(state: dict) -> dict:
    ...
    try:
        ...
        # ⚠️ HITL Trigger Logic (TIP-004)
        if result["confidence"] < 0.7:
            state["hitl_triggered"] = True
            state["history"].append(f"[{WORKER_NAME}] Confidence thấp ({result['confidence']}) -> triggered hitl_triggered=True")
        ...
        state["history"].append(
            ...
            f"sources={result['sources']}, hitl={state.get('hitl_triggered', False)}"
        )
